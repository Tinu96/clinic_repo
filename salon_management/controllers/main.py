...
class SalonBookingWeb(http.Controller):
    ...
    def chair_info(self, **post):
        salon_service_obj = request.env['salon.service'].search([])
        service_categ_obj = request.env['service.category'].search([])
        ...
        if request.env.user.tz:
            ...
            date_start = fields.Date.context_today(request)
            date_end = fields.Date.context_today(request)
        ...
        return request.render(
            'salon_management.salon_booking_form', {
                ...
                'service_categ': service_categ_obj,
